...
%%%%We assume the viral load in the lung is two magnitue larger than serum
p2=plot(t/24-incubation, 6-2+log10(y(:, 14)),'LineWidth', 2)
hold on 
...
%%%%We assume the viral load in the lung is two magnitue larger than serum
p3=plot(t/24-incubation, 6-2+log10(y2(:, 14)),'LineWidth', 2)
hold on

ylabel('Viral load (log_{10})','FontSize', 20)
xlabel('days','FontSize', 20)
set(gca,'fontsize',16)
legend([p1, p2, p3],{'\rho_T=.06','\rho_T=.12','\rho_T=.24'},'Location','southeast','FontSize', 16)
exportgraphics(gcf,'Viral_Toxin_production.eps')
...
%%%%use the estimated paprameters to graph the solutions
rhoT=.12;
funfunfun2=@(t, y) model1(t,y,rhoX,rhoT,pp, A1_Initial, A2_Initial)
[t,y2] = ode15s(funfunfun2, [0 24*(T+incubation)], y0);
...
rhoT=.24;
funfunfun2=@(t, y) model1(t,y,rhoX,rhoT,pp, A1_Initial, A2_Initial)
[t,y3] = ode15s(funfunfun2, [0 24*(T+incubation)], y0);
...
set(gca,'fontsize',16)
legend([p1, p2, p3],{'\rho_T=.06','\rho_T=.12','\rho_T=.24'},'Location','southeast','FontSize', 16)
exportgraphics(gcf,'Viral_Toxin_A2_Cells.eps')

hold off 


%%%%%The model 
function dydt = model1(t,y,rhoX,rhoT,pp, A1_Initial, A2_Initial)    %%%%%%%%%%%%%%
